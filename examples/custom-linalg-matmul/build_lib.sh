...
OBJECT_PATH="${BUILD_DIR}/gemmini_matmul.o"
LIBRARY_PATH="${SCRIPT_DIR}/libtriton_chipyard.a"
mkdir -p "${BUILD_DIR}"
...
"${RISCV_GXX}" \
  ...
  -O2 \
  -std=gnu++17 \
  -c "${SCRIPT_DIR}/gemmini_matmul.cpp" \
  -o "${OBJECT_PATH}"

"${RISCV_AR}" rcs "${LIBRARY_PATH}" "${OBJECT_PATH}"
echo "[custom-linalg-matmul] wrote ${LIBRARY_PATH}"
