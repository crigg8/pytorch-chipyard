...
Default:
  ...
  then collect the latest model.log and autotune.log under
  scripts/figures/results-workload/<workload>/, and copy those logs to
  examples/.logs. CNN and sampled-kernel workloads also collect output.bin;
  LLM workloads run with --no-output and do not require it.
